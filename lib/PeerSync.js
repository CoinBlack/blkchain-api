...
PeerSync.prototype.load_peers = function() {
  this.peerdb = [{
    ipv4: config.blackcoind.host,
    port: config.blackcoind.p2pPort
  }];
...
// PeerSync.prototype.handleAlert = function(info) {
//   console.log(info);
// };

PeerSync.prototype.handleTx = function(info) {
  var self = this;
  // console.log('handleTx', info.message.tx);
...
PeerSync.prototype.handleCheckpoint = function(info) {
  var self = this;
  // console.log(info.message);

  self.sync.storeCheckpoint(info.message.checkpoint, function(err) {
    if (err) {
      console.log('[p2p_sync] Checkpoint storing failed', err);
    }
  });
};
...
PeerSync.prototype.run = function() {
  ...
  this.peerman.on('connection', function(conn) {
    ...
    conn.on('checkpoint', self.handleCheckpoint.bind(self));
    // conn.on('alert', self.handleAlert.bind(self));
  });
