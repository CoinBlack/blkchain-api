...
var CHECKPOINT               = 'bcp-';     // last block on the chain
...
BlockDb.prototype.getCheckpoint = function(cb) {
  db.get(CHECKPOINT, function(err, val) {
    return cb(err,val);
  });
};

BlockDb.prototype.setCheckpoint = function(hash, cb) {
  db.put(CHECKPOINT, hash, function(err) {
    return cb(err);
  });
};
...
BlockDb.prototype.getLatestBlocks = function(start_ts, limit, cb) {
  var list = [];
  db.createReadStream({
    start: TIMESTAMP_PREFIX + start_ts,
    limit: limit,
