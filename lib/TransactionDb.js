...
var BAL_ADDR_PREFIX = 'txba-'; //txab-<btc_sat>-<addr> => ts:change
var UPD_ADDR_PREFIX = 'txua-'; //txa-<0/1>-<addr> => ts ; 0 - need to be updated / 1 - updated

var TIMESTAMP_PREFIX  = 'tts-';     // tts-<ts> => <txid>

// TODO: use bitcore networks module
var genesisTXID = '12630d16a97f24b287c8c2594dda5fb98c9e6c70fc61d44191931ea2aa08dc90';
var CONCURRENCY = 10;
  ...
  this.fromAddr(addr, function(err,txOut){
    ...
    txOut.forEach(function(txItem){
      ...
      if ( !seen[txItem.txid] ) {
        ...
        seen[txItem.txid]=1;
        add=1;
        address.maxTs = Math.max(address.maxTs, txItem.ts);
      }
      ...
      if (txItem.spentTxId && !seen[txItem.spentTxId]  ) {
        ...
        addSpend=1;
        address.maxTs = Math.max(address.maxTs, txItem.spentTs);
      }
    ...
    db.get(ADDR_BAL_PREFIX + addr, function (err, value) {
      ...
      address.change = (((address.balanceSat - value) / value * 100).toFixed(8) || '---') + '%';
      // address.change = ((address.change / (address.balanceSat - address.change) * 100) || 0).toFixed(8) + '%';
      // console.log(address);
      ...
      db.batch()
        ...
        .put(BAL_ADDR_PREFIX + balanceStr + '-' + addr, address.maxTs + ':' + address.change)
        .put(ADDR_BAL_PREFIX + addr, balanceStr)
        .del(UPD_ADDR_PREFIX + '0-' + addr)
        .put(UPD_ADDR_PREFIX + '1-' + addr, address.maxTs)
        .write(function (err) {
      ...
      // async.series([
      //   function (cb) {
      //     db.del(BAL_ADDR_PREFIX + value + '-' + addr, {}, cb);
      //   },
      //   function (cb) {
      //     db.put(BAL_ADDR_PREFIX + balanceStr + '-' + addr, address.maxTs + ':' + address.change, {}, cb);
      //   },
      //   function (cb) {
      //     db.put(ADDR_BAL_PREFIX + addr, balanceStr, {}, cb);
      //   },
      //   function (cb) {
      //     db.del(UPD_ADDR_PREFIX + '0-' + addr, {}, cb);
      //   },
      //   function (cb) {
      //     db.put(UPD_ADDR_PREFIX + '1-' + addr, address.maxTs, {}, cb);
      //   }
      // ],
      // function (err) {
      //   if (cb) { cb(err, address, txs);}
...
var interval;
//txa-<0/1>-<addr> => ts
TransactionDb.prototype.updateAllAddresses = function() {
  var self = this;
  // var addrUpd = [];
  var count = 0;
  var UAAStart = Date.now();
  clearInterval(interval);
  console.log(Date(UAAStart), 'Starting all address balances sync');
  // if (typeof gc !== 'undefined') {
  //   console.log(typeof gc);
  //   gc();
  //   console.log(Date(UAAStart), 'start', 'heapUsed:', toMb(process.memoryUsage().heapUsed), 'rss:', toMb(process.memoryUsage().rss));
  // }
  console.log(Date(UAAStart), 'memoryUsage - onStart', 'heapUsed:', toMb(process.memoryUsage().heapUsed), 'rss:', toMb(process.memoryUsage().rss));
  db.createReadStream({
    ...
    .on('data', function(data) {
      ...
      self.updateAddress(addr, true, function (/*err*/) {
        ...
        if (count % 100 === 0) {
          ...
          console.log(Date(Date.now()), 'memoryUsage - onData', 'heapUsed:', toMb(process.memoryUsage().heapUsed), 'rss:', toMb(process.memoryUsage().rss));
        }
      });
    })
    .on('error', function(err) {
    ...
    .on('end', function(err) {
      ...
      var UAAEnd = Date.now();
      console.log(Date(UAAEnd), 'Updated ' + count + ' address balances in ' + ~~((UAAEnd - UAAStart) / 1000) + ' seconds');
      console.log(Date(UAAEnd), 'memoryUsage - onEnd', 'heapUsed:', toMb(process.memoryUsage().heapUsed), 'rss:', toMb(process.memoryUsage().rss));
      // db.close(function () {
      //   db = levelup(config.leveldb + '/txs',{maxOpenFiles: MAX_OPEN_FILES} );
      ...
      // if (typeof gc !== 'undefined') {
      //   gc();
      //   console.log(Date(UAAEnd), 'end', 'heapUsed:', toMb(process.memoryUsage().heapUsed), 'rss:', toMb(process.memoryUsage().rss));
      // }
      // setTimeout(function () {
      //   self.updateAllAddresses();
      // }, 1000 * 60);
...
TransactionDb.prototype.refreshAllAddresses = function() {
  // var self = this;
  // var addrUpd = [];
  var count = 0;
  var UAAStart = Date.now();
  console.log(Date(UAAStart), 'Starting all address balances refreshing');
  db.createReadStream({
    start: UPD_ADDR_PREFIX + '1-',
    end: UPD_ADDR_PREFIX + '1-~'
  })
    .on('data', function(data) {
      var arr = data.key.split('-');
      count++;
      db.batch()
        ...
        .write(function (err) {
          if (err) {console.log(err);}
          if (count % 100 === 0) {
            console.log(count + ' address balances set to be refreshed so far');
          }
        });
    })
    .on('error', function(err) {
    ...
    .on('end', function(err) {
      ...
      // console.log('addrUpd', addrUpd);
      var UAAEnd = Date.now();
      console.log(Date(UAAEnd), 'Refreshed ' + count + ' address balances in ' + ~~((UAAEnd - UAAStart) / 1000) + ' seconds');
      // cb(err, count);
      // return c(err, ret);
    });
};
...
TransactionDb.prototype.top100 = function(cb) {
  ...
  db.createReadStream({
    ...
    .on('data', function(data) {
      ...
      var arrKey = data.key.split('-');
      var arrValue = data.value.split(':');
      result.push({
        balance: (+arrKey[1] / util.COIN).format(8, 3, ' ', '.'),
        address: arrKey[2],
        ts: arrValue[0],
        change: arrValue[1]
      });
