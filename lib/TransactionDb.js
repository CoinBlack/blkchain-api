...
TransactionDb.prototype.updateAddress = function(addr, notxlist, cb) {
  ...
  this.fromAddr(addr, function(err,txOut){
    ...
    db.get(ADDR_BAL_PREFIX + addr, function (err, value) {
      ...
      //address.change = (((address.balanceSat - value) / value * 100).toFixed(8) || 0) + '%';
...
TransactionDb.prototype.updateAllAddresses = function() {
  ...
  db.createReadStream({
    ...
    .on('end', function(err) {
      ...
      console.log(new Date(), 'Updated ' + count + ' address balances in ' + ~~((Date.now() - UAAstart) / 1000) + ' seconds');
      // setTimeout(function () {
...
TransactionDb.prototype.refreshAllAddresses = function() {
  ...
  db.createReadStream({
    ...
    .on('end', function(err) {
      if (err) {console.log(err);}
      // console.log('addrUpd', addrUpd);
      console.log(new Date(), 'Refreshed ' + count + ' address balances in ' + ~~((Date.now() - UAAstart) / 1000) + ' seconds');
      // cb(err, count);
      // return c(err, ret);
    });
};
