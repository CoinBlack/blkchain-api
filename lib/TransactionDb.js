...
var redisTest = require('../test/redis.js');
...
TransactionDb.prototype.updateAddress = function(addr, notxlist, cb) {
  ...
  this.fromAddr(addr, function(err,txOut){
    ...
    redisTest.redisClient.zadd('testAddrZet', address.balanceSat, addr);
    if (cb) { cb(null, address, txs);}
  });
};
...
// console.log(tx.confirmations);
  redisTest.redisClient.zadd('TxZet', tx.confirmations || 0, tx.txid, function (err, result) {
    if (err) {
      // console.log(err, 'TxZet', tx.txid, tx.confirmations);
      errs.push(err + ' TxZet ' + tx.txid + ' = ' + tx.confirmations);
      console.log(errs);
    }
  });

// if (tx.confirmations > 10) {
//   // redisTest.redisClient.zadd('testTxZet', tx.confirmations, tx.txid, function (err, result) {
        ...
        function(i, next_out) {
          db.batch()
            .put(SPENT_PREFIX + i.txid + '-' + i.vout + '-' + tx.txid + '-' + i.n,
              ts || 0)
            .write(next_out);
            if (tx.confirmations && tx.confirmations > 10) {
              redisTest.redisClient.del('TxOut:' + i.txid + ':' + i.n,
                function (err, result) {
                  if (err) {
                    // console.log(err, 'TxOut:spent', tx.txid);
                    errs.push(err + ' DEL TxOut ' + tx.txid + ' = ' + i.txid);
                    console.log(errs);
                  }
                }
              );
              redisTest.redisClient.zrem('TxZet', tx.txid,
                function (err, result) {
                  if (err) {
                    // console.log(err, 'TxZet', tx.txid, tx.confirmations);
                    errs.push(err + ' ZREM TxZet ' + tx.txid + ' = ' + tx.confirmations);
                    console.log(errs);
                  }
                }
              );
            } else {
              redisTest.redisClient.hmset('TxOut:' + i.txid + ':' + i.n,
                'spent', tx.txid,
                function (err, result) {
                  if (err) {
                    // console.log(err, 'TxOut:spent', tx.txid);
                    errs.push(err + ' TxOut:spent ' + tx.txid + ' = ' + i.txid);
                    console.log(errs);
                  }
                }
              );
            ...
            db.get(k, function(err) {
              ...
              if (err && err.notFound) {
                db.batch()
                  ...
                  .write(next_out);
                // self.updateAddress(addr, true);
                redisTest.redisClient.hmset('TxOut:' + tx.txid + ':' + o.n,
                  'address', addr,
                  'value', sat,
                  function (err, result) {
                    if (err) {
                      // console.log(err, 'TxOut:address_value', tx.txid);
                      errs.push(err + ' TxOut:address_value ' + tx.txid + ':' + o.n);
                      console.log(errs);
                    }
                  }
                );
