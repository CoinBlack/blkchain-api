...
var BAD_GEN_ERROR = 'Bad genesis block. Network mismatch between Insight and blackcoind? Insight is configured for:';
...
function HistoricSync(opts) {
  ...
  this.rpc = new RpcClient(config.blackcoind);
...
HistoricSync.prototype.prepareFileSync = function(opts, next) {
  ...
  if ( opts.forceRPC || !config.blackcoind.dataDir ||
    self.connectedCountDB > self.blockChainHeight * 0.9) return next();


  try {
  self.blockExtractor = new BlockExtractor(config.blackcoind.dataDir, config.network);
  } catch (e) {
...
HistoricSync.prototype.showSyncStartMessage = function() {
  ...
  p('Got ' + self.connectedCountDB +
    ' blocks in current DB, out of ' + self.blockChainHeight + ' block at blackcoind');

  if (self.blockExtractor) {
    p('blackcoind dataDir configured...importing blocks from .dat files');
...
HistoricSync.prototype.setupSyncStatus = function() {
  ...
  var step = parseInt( (self.blockChainHeight - self.syncedBlocks) / SYN_STEPS);
      ...
      function (w_cb) {
        self.getFn(function(err,blockInfo) {
          ...
          if (blockInfo && blockInfo.hash) {
            // self.syncedBlocks++;
            self.syncedBlocks = blockInfo.height;// + 1;
...
HistoricSync.prototype.updateAllAddresses = function () {
  var self = this;
  var UAAstart = Date.now();
  p(Date(UAAstart), 'Starting all address balances sync');
  // console.log(self.sync);
  self.sync.txDb.updateAllAddresses(function (err, count) {
    if (err) {console.log(err);}
    p(new Date(), 'Updated ' + count + ' address balances in ' + ~~((Date.now() - UAAstart) / 1000) + ' seconds');
    setTimeout(function () {
...
HistoricSync.prototype.refreshAllAddresses = function () {
  var self = this;
  var UAAstart = Date.now();
  p(Date(UAAstart), 'Starting all address balances refreshing');
  // console.log(self.sync);
  self.sync.txDb.refreshAllAddresses(function (err, count) {
    if (err) {console.log(err);}
    p(new Date(), 'Refreshed ' + count + ' address balances in ' + ~~((Date.now() - UAAstart) / 1000) + ' seconds');
  });
};
